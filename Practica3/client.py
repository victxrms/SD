...
def login():
    ...
    response = requests.get('http://localhost:8080/login', 
                             json = {'userName': username, 'password': password})   #imprime la respuesta del servidor tras solicitar el login
    print(response.text)
...
def menu():
    while True:
        ...
        if menu == 3:   #si es 3
            ...
            if DNIaux != 'False':
                print ("Login correcto.\n")
                addBook(DNIaux) #llama a la funcion addBook
            else:
                print ("Login fallido.\n")
        ...
        if menu == 4:   #si es 4
            ...
            if DNIaux != False:
                print ("Login correcto.\n")
                response = requests.get('http://localhost:8080/showBookings/' + DNIaux)    #guarda en response la respuesta que recibe del del servidor al enviar la peticion
                print (response.text)                                                   #la imprime
            else:
                print ("Login fallido.\n")
...
def addBook(DNIaux):
    ...
    fechaSTR = input("Fecha en formato dd/mm/aaaa: ")
    hora = input("Hora en formato: hh:mm ")
    duracion = int(input("Duración en minutos: "))
    fecha = datetime.datetime.strptime(fechaSTR,'%d/%m/%Y').strftime('%d/%m/%Y')
    ...
    horaFin = (t1 + t2).strftime('%H:%M')                                                                                                               #calcula la hora fin con la suma de la hora inicio y los minutos
    response = requests.post('http://localhost:8080/addBooking', 
                             json={'DNI' : DNIaux, 'salaId':salaId ,'date': fecha, 'startTime': hora, 'duration': duracion, 'endTime': str(horaFin)[0:5]}) #guarda en response la respuesta que recibe del del servidor al enviar la peticion
    if response.text != '':                                                                                                                             #si es distinto de 0, es decir, hemos recibido algun dato, esperando que sea el de error y la lista
        print (response.text)                                                                                                                           #imprimos la respuesta
        addBook(DNIaux)                                                                                                                                 #llamamos de nuevo al a funcion addBook para que el ususario tras haber visto la lista de salas libres en su horario decida una nueva 
